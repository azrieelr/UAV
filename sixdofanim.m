function [sys,x0,str,ts,simStateCompliance] = sixdofanim(t,x,u,flag,Config)
...
function [x,y,z]=miss_shape
	...
	num=15;		     % Number of z-y segments to make the circle
    num=10;		     % Number of z-y segments to make the circle
%	count=1;	     % Counter for number of individual patches
        xcraft = cell(50,1); %Preallocate for current number of patches
        ycraft = cell(50,1); %Preallocate for current number of patches
        zcraft = cell(50,1); %Preallocate for current number of patches
        
	theta=(360/2/num:360/num:(360+360/2/num))*pi/180;

	len 			= 24.5;				% Total Length (no units)
	radius		= 19/20;				% Radius of body
 	s_fore	 	= 1.2;					% Start of main body (w.r.t. nose)
   	thr_len 		= 1.5;				% Length of Motor exit
 	rad_throt	= 3/6;				% radius of motor exit
   	l_fore=len-s_fore-thr_len;			% Length of main body
   	c_g 			= 23;				% Position of c.g. w.r.t nose
...
%
	for i = 2:num
	   xcraft{count}=[s_fore s_fore -1.5 s_fore]-c_g;
...
% Wing shapes 
%
	xcraft{count}=[9 13 15 15]-c_g;
	ycraft{count}=[-zc_range(1) -zc_range(1)+12 -zc_range(1)+12 -zc_range(1)];
	zcraft{count}=[0 0 0 0];
	xcraft{count+3}=xcraft{count};
	ycraft{count+3}=-ycraft{count};
	zcraft{count+3}=zcraft{count};
	xcraft{count+5}=xcraft{count};
	ycraft{count+5}=zcraft{count};
	zcraft{count+5}=ycraft{count};
	xcraft{count+5}=xcraft{count};
	ycraft{count+5}=zcraft{count};
	zcraft{count+5}=-ycraft{count};
%
...
%
   count=count+1;
	...
	xcraft{count+4}=xcraft{count};
	ycraft{count+4}=zcraft{count};
	zcraft{count+4}=-ycraft{count};
   	count=count+3;
